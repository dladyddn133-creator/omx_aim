...
실제 로봇 운용 환경에서 표적 영상 10개 클립을 촬영하고,
일정 프레임 간격으로 이미지를 추출하여 단일 클래스(`target`) 데이터셋을 직접 구축했습니다.

<div align="center">

<img src="docs/media/yolo/train_batch0.jpg"
     alt="Training dataset samples with labels"
     width="820">

</div>

데이터셋 구성 시 다음 조건을 의도적으로 포함했습니다.

| 조건 | 목적 |
|---|---|
| 근거리 ~ 원거리 표적 | 조준 거리에 따른 검출 크기 변화 대응 |
| 다양한 촬영 각도 및 부분 가림 | 실제 주행 중 발생하는 시야 제약 대응 |
| 모션 블러 포함 프레임 | 로봇 이동 및 로봇팔 회전 중 검출 안정성 확보 |
| 조명 및 배경 변화 | 강의실, 복도, 창가 등 운용 환경 차이 대응 |
| **네거티브 샘플 포함** | 표적이 없는 장면에서의 오탐 억제 |

특히 자동 타격 시스템 특성상 **오탐이 곧 오발사로 이어지므로**,
표적이 존재하지 않는 장면을 별도로 수집하여 학습에 포함했습니다.
